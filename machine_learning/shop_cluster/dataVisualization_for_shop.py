...
def thumb_withparams(params,imglist,label,variance,output):
    picsize=150
    cur_width = 0
    cur_height= 20
    nth=0
...
def get_worst_v_nearest_clusters(features,images):
    variances=[]
    for value in images.values():
        variances.append(np.sum(np.var(features[value,],axis=0)))
#        variances=np.sum(np.var(features[images.values(),],axis=1),axis=1)
...
def get_label_Varience(features,images):
    variances=[]
    for value in images.values():
        variances.append(np.sum(np.var(features[value,],axis=0)))
#        variances=np.sum(np.var(features[images.values(),],axis=1),axis=1)
    variances=np.array(variances)
    print variances.shape
    f = open('label_varience_0.5_0.1_40','w')
    for variance, label in sorted(zip(variances, images.keys())):
        f.write(str(label)+' '+str(variance)+'\n')
    f.close()
    print 'end write label and variance'
...
def main( feature_file,labels_file,align_file,tagdest,kind): 
   ...
   #     get_label_Varience(features,images)
        worstClusters,V=get_worst_v_nearest_clusters(features,images)
