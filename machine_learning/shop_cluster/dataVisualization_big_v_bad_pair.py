...
def thumb(params,imglist,label,variance,output):
    picsize=150
    cur_width = 0
    cur_height= 20
    nth=0
...
#    for img in imglist:
    length = len(imglist)
    step = length/50 + 1
    font2 = ImageFont.truetype('/usr/share/fonts/truetype/dejavu/DejaVuSerif-Bold.ttf', 40)
    for index in xrange(0, len(imglist) ,step):
       # img = name_align_dict[img]
       # print img
        img = imglist[index]
        param_tmp=params[index]
        nth += 1
       ...
       #     os.system("wget http://192.168.3.253:9333/"+img+" -P imgs/ >/dev/null 2>&1")
        ...
        num = 140
        for i in param_tmp:
            dl.text((0, num), i, fill='blue', font=font2)
            num+=40
        if img.mode != 'RGB':
            img = img.convert('RGB')
...
def merge(llabel_a, rlabel_b,lid, rid,dist,output_file, l_p_id, r_p_id, l_params,r_params,l_daibiao,r_daibiao):  
   ...
   # height=720
    ...
    msg1="Dist = "+str(round(dist,3))+'   Consine_dist:'+str(round(consine_dist,3))+'       l_label: '+llabel_a+'nums:'+str(len(l_p_id))+'           r_label: '+rlabel_b+'nums:'+str(len(r_p_id))
    dl.text((0,0),msg1, fill="red",font=font)
...
def read(path, path_label,path_align):
    ...
    with open(path_align) as r:
        ...
        while line:
            line_ary=line.split()
            name_param_dict[line_ary[0]]=line_ary[1].split(',')
            line = r.readline().strip()
...
#    f = open('label_varience','w')
#    for variance, label in sorted(zip(variances, images.keys())):
#        f.write(str(label)+' '+str(variance)+'\n')
#    f.close()
#    print 'end write label and variance'
    indexs=np.argpartition(-variances,100)[:100]
#    thre=0.6
#    indexs=np.where((variances>thre)&(variances<thre+0.005))[0][:100]
    return np.array(images.keys())[indexs],variances[indexs]


def get_pairs(features,images):
    variances=[]
    thr = 0.75
    cosine=0.502
    l_pic=[]
    ...
    for value in images.values():
       # variances.append(np.sum(np.var(features[value,],axis=0)))
        vr = np.sum(np.var(features[value,],axis=0))
        if vr > thr:
            mat=1-cdist(features[value,],features[value,],metric="cosine")
            wh=np.where((mat>cosine)&(mat<0.55))
            for i in range(len(wh[0])):
                l_pic.append(value[wh[0][i]])
                r_pic.append(value[wh[1][i]])
                v.append(vr)
            num+=1
            if num >10000:
                break
    return l_pic, r_pic, v
...
def do2(a):
    return min(1.4084507*(1+float(a))/2-0.17746479,0.99)


def merge_2(pic1,pic2,info1,l_param,r_param,output):
    merge_img = Image.new('RGB', (800,528), 0xffffff)
    title='result/shoptest/'
    pic1=title+pic1
    pic2=title+pic2
...
#    merge_img = paste(merge_img,pic3,(800,20))
    ...
    draw.text((0,0),"cos = "+str(round(info1,3)), fill="red",font=font)
   # print l_param
    l_tmp1 = list(l_param[:2])+list(l_param[3:7])
    l_tmp2=l_param[-6:]
    r_tmp1=list(r_param[:2])+list(r_param[3:7])
    r_tmp2=r_param[-6:]
    num=0
    for i in ['detect:','align:','angle1:','blur:','angle2:','age']:
        draw.text((0,420+num),i,fill='red',font=font)
        num+=18
    num=0
    for i in l_tmp1:
        draw.text((150,420+num),i,fill="red",font=font)
        num+=18
    num=0
    for i in l_tmp2:
        draw.text((300,420+num),i,fill='red',font=font)
        num+=18
    num=0
    for i in r_tmp1:
        draw.text((400,420+num),i,fill='red',font=font)
        num+=18
    num=0
    for i in r_tmp2:
        draw.text((550,420+num),i,fill='red',font=font)
        num+=18
...
def check(lst,v,kind):
	for i in lst:
		if i=="None" or (abs(float(i))<v)==kind:
			return 0
	return 1
...
def main( feature_file,labels_file,align_file,tagdest,kind): 
    ...
    if kind&1:
        ...
        for cluster in worstClusters[:100]:
           # thumb(names[images[cluster]],V[cnt],os.path.join(tagdest,"1_"+str(cnt)+".jpg"))
            thumb(params[images[cluster]],names[images[cluster]],cluster,V[cnt],os.path.join(tagdest,"1_"+str(cnt)+".jpg"))
            cnt+=1
    ...
    if kind&4:
        if not os.path.exists('pairs'):
            os.mkdir('pairs')
        cnt=0
        l_pic,r_pic,V=get_pairs(features,images)
        for l,r,v,l_param,r_param,f1,f2 in zip(names[l_pic],names[r_pic],V,params[l_pic],params[r_pic],features[l_pic],features[r_pic]):
            l_detect=l_param[0]
            r_detect=r_param[0]
            l_align=l_param[1]
            r_align=r_param[1]
            ...
            l_angle2=l_param[5]
            r_angle2=r_param[5]
            if not check([l_detect,r_detect],0.6,1) or not check([l_align,r_align],0.2,1) or not check([l_angle1,r_angle1,l_angle2,r_angle2],40,0):
                continue
            out_put='pairs/'+str(cnt)+'.jpg'
            simi=np.sum(f1*f2)
            merge_2(l,r,do2(simi),l_param,r_param,out_put)
            cnt+=1
            if cnt > 1000:
                break
