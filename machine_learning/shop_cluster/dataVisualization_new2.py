...
def thumb(pid, label_before, imglist,label,variance,output):
    picsize=150
    cur_width = 0
    cur_height= 20
    nth=0
...
#    for img in imglist:
    ...
    font2 = ImageFont.truetype('/usr/share/fonts/truetype/dejavu/DejaVuSerif-Bold.ttf', 30)
    for index in xrange(0, len(imglist) ,step):
       # img = name_align_dict[img]
       # print img
        img = imglist[index]
        label_tmp = label_before[index]
        pid_tmp = pid[index].split('/')[-1].split('_')
        nth += 1
        ...
        dl.text((0, 0), label_tmp, fill='red', font=font)
        num = 200
        for i in pid_tmp:
            dl.text((0, num), i, fill='blue', font=font2)
            num+=30
        #dl.text((0, 350), pid_tmp, fill='blue', font=font2)
        if img.mode != 'RGB':
            img = img.convert('RGB')
...
def read(path, path_label, path_label_before,path_align):
    name_label_dict={}
    name_align_dict={}
    name_param_dict={}
    name_label_before_dict={}
    with open(path_label) as r:
        ...
        while line:
            ...
            line=r.readline().strip()
    with open(path_label_before) as r:
        line = r.readline().strip()
        while line:
            ary = line.split(' ')
            name_label_before_dict[ary[0]]=ary[1]
            line=r.readline().strip()
...
#            images.pop(last)
  ...
  #2          continue
        ...
        labels.append(this)
        labels_before.append(name_label_before_dict[array[0]])
       # features.append(map(float,array[2:]))
        ...
        if i>500000:
            break
    return np.array(names),images,np.array(labels),np.array(labels_before),normalize(np.array(features,dtype="f")),np.array(p_id),np.array(params)
...
def main( feature_file,labels_file,labels_file_before,align_file,tagdest,kind):
    names,images,labels,labels_before,features,p_id,params=read(feature_file, labels_file,labels_file_before, align_file)
    print "read!",time()
    ...
    if kind&1:
        ...
        for cluster in worstClusters[:100]:
           # thumb(names[images[cluster]],V[cnt],os.path.join(tagdest,"1_"+str(cnt)+".jpg"))
            thumb(p_id[images[cluster]],labels_before[images[cluster]],names[images[cluster]],cluster,V[cnt],os.path.join(tagdest,"1_"+str(cnt)+".jpg"))
            cnt+=1
...
if __name__ == "__main__":
    dic = dict()
    dic['features_file'] = './zhangwei/zhangwei_featlst'
    dic['labels_file_2'] = 'labels_0.5_0.7_1.0s_by_npy_not_merge'
    dic['labels_file_1'] = 'labels_0.7'
    dic['parameters_file'] = './zhangwei/zhangwei_parameters'
    dic['out_put'] = dic['labels_file_2'].split('ls_')[1]
    ...
    with open('datavisual_params') as f:
        ...
        print duixiang
    feature_file=duixiang['features_file'] # xxx_featlst
    labels_file=duixiang['labels_file_2'] # labels_xxx
    labels_file_before=duixiang['labels_file_1'] # old_labels_xxx
    align_file=duixiang['parameters_file'] # xxx_paramerters
    tagdest=duixiang['out_put']
    ...
    if len(sys.argv)>6:
        ...
        kind=3
    main(feature_file,labels_file,labels_file_before, align_file,tagdest,kind)
