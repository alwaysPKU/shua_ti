...
def merge(llabel_a, rlabel_b,lid, rid,dist,output_file, l_p_id, r_p_id):  
    width=1280
    height=720
    merge_img = Image.new('RGB', (width,height), 0xffffff)
...
#            images.pop(last)
  ...
  #2          continue
       ...
       # features.append(map(float,array[2:]))
        ...
        if i>200000:
            break
    return np.array(names),images,np.array(labels),normalize(np.array(features,dtype="f")),np.array(p_id)
...
def main( feature_file,labels_file,align_file,tagdest,kind): 
    names,images,labels,features,p_id=read(feature_file, labels_file, align_file)
    print "read!",time()
    ...
    if kind&2:
        ...
        for a,b,dist in sorted(zip(*results)): 
            if a==lasta and b==lastb:
                continue
            merge(a, b, names[images[a]],names[images[b]],dist,os.path.join(tagdest,"2_"+str(cnt)+".jpg"),p_id[images[a]], p_id[images[b]])
            cnt+=1
            lasta=a
            lastb=b
            if cnt>100:
