...
def main(fid_label_file,out_put):
    ...
    for a, b, dist, l_index, r_index in sorted(zip(*results)):
       # print a,b,dist,l_index,r_index
        if a == last_a and b == last_b:
            continue
        delta = time_window(l_index, r_index, fids)
        if not delta:
            continue
        if delta==1:
            if [a, b] not in labels_fix:
            ...
            last_b=b
    t2 = datetime.datetime.now()
    print 'end find merge label:', t2, t2-t1, '\n'
    print 'tongyizhen:',len(dele_fix)
    print 'dele_before:',len(labels_fix)
    labels_fix = [v for v in labels_fix if v not in dele_fix]
    print 'dele_after:', len(labels_fix)
...
if __name__ == '__main__':
    ...
    with open('cluster_2_params') as f:
        ...
        out_put = 'labels_'+str(s_c_c_d)+'_0.7_'+str(time_threshold)+'s_dele'
