...
struct is_true
{
    ...
    int *dev_timeq,*dev_timeg;
    int diffA,diffB,lb;
    
    is_true(float _threshold,int* _dev_timeq, int* _dev_timeg,int _diffA,int _diffB, int _lb) : threshold(_threshold),dev_timeq(_dev_timeq),dev_timeg(_dev_timeg),diffA(_diffA),diffB(_diffB),lb(_lb) {}

  __host__ __device__
  bool operator()( const thrust::pair    <float,int> &x )
  {
    // return (x.first>=Threshold)&&(x.first <0.9999);
    return (x.first>= threshold)&&(std::abs(dev_timeq[diffA+x.second/lb]-dev_timeg[diffB+x.second%lb])<=1);
  }
};
...
extern "C" void calcthreshold( float* A, float* B,int* paras,float threshold, int* index1,int* index2,float* scores,int* timeq,int* timeg) {
...
//    cudaDeviceSetLimit(cudaLimitMallocHeapSize, ((uint64_t)4*alloc_size*alloc_size)); 
    ...
    CHECK_CUBLAS_ERROR(cudaMalloc((void**)&dev_timeq, (l1)*sizeof(float)));
    CHECK_CUBLAS_ERROR(cudaMalloc((void**)&dev_timeg, (l2)*sizeof(float)));    


    cudaMemcpy(dev_timeq,timeq,l1*sizeof(float),cudaMemcpyDeviceToHost);
    cudaMemcpy(dev_timeg,timeg,l2*sizeof(float),cudaMemcpyDeviceToHost);
...
//    printf("l1=%d, l2=%d\n",l1,l2);   
    ...
    for ( int i=0;i<ceili;i++){//ceili
        int lb=min(l2-alloc_sizeB*i,alloc_sizeB);
        CHECK_CUBLAS_ERROR(cublasSetMatrix(lb, dim, sizeof(float), B+(uint64_t)alloc_sizeB*i*dim, lb, dev_mat2, lb));
        for ( int j=0;j<ceilj;j++){//ceilj
             clock_t t22 = clock();
            int la=min(l1-alloc_sizeA*j,alloc_sizeA);
           // printf("i=%d , j= %d , la=%d, lb=%d v=%f\n",i,j,la,lb,*A);
            CHECK_CUBLAS_ERROR(cublasSetMatrix(la, dim, sizeof(float), A+(uint64_t)alloc_sizeA*j*dim, la, dev_mat1, la));
...
//            printf("12321321313\n");
            offset = thrust::copy_if(dv, dv+la*lb, tv, is_true(threshold,dev_timeq,dev_timeg,j*alloc_sizeA,alloc_sizeB*i,lb)) - tv ;
            sum+=offset;
//            printf("offset = %d\n",offset);
            ...
            {//   printf("%d %d %d\n",pleft,pright,batches);
                ...
                {
                    index1[paras[4]]=lvalue+j*alloc_sizeA+h_vector[it].second/lb;
                    index2[paras[4]]=i*alloc_sizeB+h_vector[it].second%lb;
